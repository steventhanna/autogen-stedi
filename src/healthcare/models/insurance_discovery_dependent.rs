...
pub struct InsuranceDiscoveryDependent {
    ...
    /// The dependent's Social Security Number (SSN).   - **Medical coverage:** The patient's full SSN is best, but even the last 4 digits of the SSN can help narrow down matching coverage.   - **Dental coverage:** Providing the SSN is especially important. Many dental payers accept SSN in place of a member ID. When included, Stedi can search with additional dental carriers that accept SSN instead of member ID. If you don't have the subscriber's member ID but can include the SSN, Stedi is more likely to return active dental coverage.
    #[serde(rename = "ssn", skip_serializing_if = "Option::is_none")]
    pub ssn: Option<String>,
}

impl InsuranceDiscoveryDependent {
    /// Demographic information for the patient when they are a dependent on a health plan. - We **strongly recommend** providing as much information as possible to improve the probability of finding matching coverage. We especially recommend including the dependent's Social Security Number and their address - particularly their state and, if known, their ZIP Code. - You should provide information for both the subscriber and the dependent in the request when possible. - If you only have the dependent's information, you should identify them in the `subscriber` object instead and leave this object empty. Note that some payers require information about both the dependent and the subscriber, so providing only the dependent's information limits Stedi's ability to return coverage matches for those payers.
