...
/// EligibilityCheckThrottledException : The request was not sent to the payer because your account's real-time concurrency limit was exceeded. Retry with backoff. The body carries the same properties as a successful response, with a synthesized 271 whose `AAA` error reports the rejection.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct EligibilityCheckThrottledException {
...
impl EligibilityCheckThrottledException {
    /// The request was not sent to the payer because your account's real-time concurrency limit was exceeded. Retry with backoff. The body carries the same properties as a successful response, with a synthesized 271 whose `AAA` error reports the rejection.
    pub fn new(id: String, message: String, payer: models::EligibilityCheckPayer, x12: String) -> EligibilityCheckThrottledException {
        EligibilityCheckThrottledException {
            dependent: None,
