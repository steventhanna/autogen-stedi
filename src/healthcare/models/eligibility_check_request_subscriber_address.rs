/*
 * Stedi Healthcare
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2024-04-01
 * Contact: healthcare@stedi.com
 * Generated by: https://openapi-generator.tech
 */

use crate::healthcare::models;
use serde::{Deserialize, Serialize};

/// EligibilityCheckRequestSubscriberAddress : The subscriber's address. When providing address information: - `addressLine1` and `city` are **required** for standard eligibility checks and MBI lookups with SSN. We also recommend including `state` for member identification. - When performing an [MBI lookup without SSN](https://www.stedi.com/docs/healthcare/mbi-lookup) (Payer ID: `MBILUNOSSN`), only `state` is required. You can omit `addressLine1` and `city`.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct EligibilityCheckRequestSubscriberAddress {
    /// The first line of the address. Required for all payers except payer ID `MBILUNOSSN`.
    #[serde(rename = "addressLine1", skip_serializing_if = "Option::is_none")]
    pub address_line1: Option<String>,
    /// The second line of the address.
    #[serde(rename = "addressLine2", skip_serializing_if = "Option::is_none")]
    pub address_line2: Option<String>,
    /// The city. Required for all payers except payer ID `MBILUNOSSN`.
    #[serde(rename = "city", skip_serializing_if = "Option::is_none")]
    pub city: Option<String>,
    /// The two-letter country code from [Part 1 of ISO 3166](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2).
    #[serde(rename = "country", skip_serializing_if = "Option::is_none")]
    pub country: Option<String>,
    /// The country subdivision code from [Part 2 of ISO 3166](https://en.wikipedia.org/wiki/ISO_3166-2).
    #[serde(rename = "countrySubdivision", skip_serializing_if = "Option::is_none")]
    pub country_subdivision: Option<String>,
    /// The United States or Canadian postal code, excluding punctuation and blanks.
    #[serde(rename = "postalCode", skip_serializing_if = "Option::is_none")]
    pub postal_code: Option<String>,
    /// The US state or Canadian province code. Required for payer ID `MBILUNOSSN`.
    #[serde(rename = "state", skip_serializing_if = "Option::is_none")]
    pub state: Option<String>,
}

impl EligibilityCheckRequestSubscriberAddress {
    /// The subscriber's address. When providing address information: - `addressLine1` and `city` are **required** for standard eligibility checks and MBI lookups with SSN. We also recommend including `state` for member identification. - When performing an [MBI lookup without SSN](https://www.stedi.com/docs/healthcare/mbi-lookup) (Payer ID: `MBILUNOSSN`), only `state` is required. You can omit `addressLine1` and `city`.
    pub fn new() -> EligibilityCheckRequestSubscriberAddress {
        EligibilityCheckRequestSubscriberAddress {
            address_line1: None,
            address_line2: None,
            city: None,
            country: None,
            country_subdivision: None,
            postal_code: None,
            state: None,
        }
    }
}

