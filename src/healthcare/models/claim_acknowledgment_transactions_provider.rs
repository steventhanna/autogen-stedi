/*
 * Stedi Healthcare
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2024-04-01
 * Contact: healthcare@stedi.com
 * Generated by: https://openapi-generator.tech
 */

use crate::healthcare::models;
use serde::{Deserialize, Serialize};

/// ClaimAcknowledgmentTransactionsProvider : Information about the provider receiving the claim status details.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ClaimAcknowledgmentTransactionsProvider {
    /// The provider's Electronic Transmitter Identification Number.
    #[serde(rename = "etin", skip_serializing_if = "Option::is_none")]
    pub etin: Option<String>,
    /// The provider's first name. Can be up to 35 characters.
    #[serde(rename = "firstName", skip_serializing_if = "Option::is_none")]
    pub first_name: Option<String>,
    /// The provider's last name. Can be up to 60 characters.
    #[serde(rename = "lastName", skip_serializing_if = "Option::is_none")]
    pub last_name: Option<String>,
    /// The provider's middle name or initial. Can be up to 25 characters.
    #[serde(rename = "middleName", skip_serializing_if = "Option::is_none")]
    pub middle_name: Option<String>,
    /// The provider's business name. Can be up to 60 characters.
    #[serde(rename = "organizationName", skip_serializing_if = "Option::is_none")]
    pub organization_name: Option<String>,
}

impl ClaimAcknowledgmentTransactionsProvider {
    /// Information about the provider receiving the claim status details.
    pub fn new() -> ClaimAcknowledgmentTransactionsProvider {
        ClaimAcknowledgmentTransactionsProvider {
            etin: None,
            first_name: None,
            last_name: None,
            middle_name: None,
            organization_name: None,
        }
    }
}

